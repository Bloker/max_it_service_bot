...
Статус: ручной smoke пройден локально для `test_dev_max` 2026-07-13; production не изменялся.
...
и кнопку «Главное меню». Фото и видео из такого ответа выводятся как в сообщении
пользователю, так и в закрытой карточке заявки в группе; временные сообщения
специалиста очищаются best-effort с повторной попыткой.
```

Результаты ручной проверки 2026-07-13:

```text
текстовая заметка БЗ: OK
заметка БЗ с фото и видео: OK
открытие вложений БЗ: OK
внутренний комментарий без текста/с текстом/с фото и видео: OK
отмена внутреннего комментария и очистка временных сообщений: OK
закрытие с ответом, фото и видео: OK
media на закрытой групповой карточке: OK
кнопка «Главное меню» после закрытия: OK, открывает Jamaica-меню
```

## Stage 5.1: дополнения пользователя и повторное открытие

Для Jamaica используется общий flow дополнений. Пользователь открывает собственную
активную заявку через `Мои заявки`, нажимает `➕ Дополнить заявку` и отправляет
text/photo/video/file. После 15 секунд дополнение сохраняется отдельно и появляется
в IT-группе как reply к основной карточке. До нажатия `Прикрепить к заявке` карточка,
статус и исполнитель не меняются.

У закрытой Jamaica-заявки специалист видит в указанном порядке:
...
При повторном открытии существующий исполнитель сохраняется; без исполнителя заявка
возвращается в статус `новое`. История номера и связь `ticket_context.location_id`
не изменяются. Кнопка внутренних комментариев скрыта, но сохранённые комментарии
и backend остаются доступными для будущей админки.
